...
{
  // Slide timer (MM:SS)
// Writes elapsed time into Storyline variable: TimerText
// Create a Storyline Text variable named "TimerText" and display it on the slide.

// --- Config ---
const OUTPUT_VAR = 'TimerText';
...
// Single update loop; counts whole seconds based on the provided time param
update((time) => {
  // time is in ms
  const wholeSecond = Math.floor(time / 1000);
  if (wholeSecond === lastWholeSecond) return;

  // On first call, set baseline
  if (lastWholeSecond === -1) {
    lastWholeSecond = wholeSecond;
    elapsedSeconds = 0;
    setVar(OUTPUT_VAR, formatMMSS(elapsedSeconds));
    return;
  }

  const deltaSeconds = Math.max(0, wholeSecond - lastWholeSecond);
  lastWholeSecond = wholeSecond;
